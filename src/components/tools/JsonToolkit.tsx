...
export function JsonToolkit() {
  ...
  const formatted = useMemo(
    () => (parsed.ok && input.trim() ? prettyPrint(parsed.value, 2) : ""),
    [input, parsed],
  );
  const showError = input.trim() !== "" && !parsed.ok;
  const hasData = input.trim() !== "" && parsed.ok;

  function format() {
    if (parsed.ok) setInput(prettyPrint(parsed.value, 2));
  }
  function minify() {
          ...
          <div className="h-[440px] overflow-auto rounded-md border border-border bg-card p-2">
            {hasData ? (
              view === "tree" ? (
                ...
                <pre className="px-1 font-mono text-xs leading-relaxed text-foreground">
