...
export function EmptyState() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute left-1/2 top-1/3 -z-10 h-[400px] w-[700px] -translate-x-1/2 rounded-full bg-brand/15 blur-[140px]" />

      <motion.div
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4 }}
        className={`relative mx-auto w-full max-w-2xl rounded-2xl border border-dashed p-10 text-center backdrop-blur transition-colors ${
          over
        ...
        <div className="relative mt-8 w-full max-w-2xl">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {recents.map((r) => (
              <div
                key={r.id}
                className="group flex items-center justify-between gap-2 rounded-lg border border-border bg-card/60 p-3 transition hover:border-brand/40 hover:bg-card"
              >
                <button
                  onClick={async () => {
                ...
                <button
                  ...
                  className="rounded p-1 text-muted-foreground opacity-0 transition group-hover:opacity-100 hover:bg-accent hover:text-destructive"
                  title="Remove"
                >
